...
export default {
    ...
    watch: {
        'formData.transaction_type': function(newValue) {
    ...
    methods: {
        ...
        temporary() {
            ...
            const apiUrl = `${this.$apiUrl}/api/get_unfinish_temporary/`;
            this.$axios.post(apiUrl, {
                userID: this.$root.$personal_id,
                account_id:this.formData.personal_account_id,
        ...
        //完成確定
        sure() {
            if (this.personal_id === '' || this.formData.item === '' || this.formData.payment === '' || this.formData.category_name === '' || this.transaction_type === '' || this.currentTime === '') {
                Swal.fire({
            ...
            const apiUrl = `${this.$apiUrl}/api/get_unfinish_sure/`;
            this.$axios.post(apiUrl, {
                userID: this.$root.$personal_id,
                account_id:this.formData.personal_account_id,
                item: this.formData.item,
        ...
        handleExpenditure() {
            this.tmp_list = this.category_list.filter(category => category.transaction_type === "支出");
        },
        handleIncome() {
            this.tmp_list = this.category_list.filter(category => category.transaction_type === "收入");
        }
    },
};
</script>
